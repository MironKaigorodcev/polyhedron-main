            ...
            for i, line in enumerate(f):
                if i == 0:
                    ...
                    self.vertexes.append(R3(x, y, z).rz(
                        alpha).ry(beta).rz(gamma) * self.scale)
                else:
        ...
        for e in self.edges:
            for f in self.facets:
                ...
                if sqrt(cent.x ** 2 + cent.y ** 2 + cent.z ** 2) > 1:
                    ...
                    for i in range(1, len(f.vertexes)-1):
                        s1 = f.vertexes[i] - f.vertexes[0]
                        s2 = f.vertexes[i+1] - f.vertexes[0]
        ...
        for f in self.facets:
            ...
            if sqrt(cent.x ** 2 + cent.y ** 2 + cent.z ** 2) > 1\
                ...
                for i in range(1, len(f.vertexes)-1):
                    s1 = f.vertexes[i] - f.vertexes[0]
                    s2 = f.vertexes[i+1] - f.vertexes[0]
                    _area += abs(s1.cross(s2).dot(norm)) / 2
        return _area